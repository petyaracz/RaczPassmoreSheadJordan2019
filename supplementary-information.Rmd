...
table_style = 'pandoc'
table_width = TRUE # True for PDF, False for HTML
```
...
Frequency data were collected from `r ncorpora` corpora in `r nlangfreq` languages in three corpus types: spoken, written, and web-crawled. The list of corpora is in Table 1 below.

```{r}
corpora %>% 
  mutate(ref = paste('[@', bibtex, ']', sep = '')) %>% 
  select(language,corpus.name,ref) %>%
  kable(table_style, col.names = c('Language', 'Corpus name', 'Reference'), caption = 'Table S1: Source of frequency data (source of words is Kinbank)') %>% 
  kable_styling(full_width = table_width, latex_options="scale_down")
```
...
data.frame(cn, cn_description) %>%
  kable(col.names = c("Column name", "Description"), format = table_style,
        caption = "Table S2: Supplementary table column descriptions") %>%
  kable_styling(full_width = table_width)
```
...
kable(t, col.names = c("Kinterm", "Description", "No. languages", "States"),
      format = table_style, caption = "Table S3: Each kinterm used, with counts for the number of
...
kable(cognate_detection, 
      table_style, caption = "Table S4: Precision, recall, & F-score for various LingPy cognate detection algorithms and their threshold settings.") %>%
  kable_styling(full_width = table_width)
```
...
for(i in seq_along(average_result)){
  print(kable(average_result[[i]], caption = titles[i], format = table_style, digits = 7) %>%
          kable_styling(full_width = table_width))
  cat('\n')
}
...
sd %>%
  ...
  summarise(mean.hf = mean(half.life) %>% round(0)) %>% 
    kable(col.names = c("Kin code", "Half-life (years)"), format = table_style,
...
summary(lmm.k1)$coef %>%
  kable(table_style, digits = 2, caption = 'Table S17: Summary of fixed effects for control model 1') %>% 
  kable_styling(full_width = table_width)
```

The word-level random intercepts capture word frequency across data sources.
The intercepts predict rate of change, even when controlling for word length.
...
summary(lmm.k2)$coef %>%
  kable(table_style, digits = 2, caption = 'Table S18: Summary of fixed effects for control model 1b') %>% 
  kable_styling(full_width = table_width)
```

## Predictive model: rate of change and frequency of use

We use the word intercepts from the control model as measures of frequency of use for kin terms, and centralised log frequency per million for the Swadesh terms. We restrict the dataset to languages for which we have kin term data.
...
summary(lmm.all1)$coef %>%
  kable(table_style, digits = 2, caption = 'Table S19: Summary of fixed effects for predictive model') %>% 
  kable_styling(full_width = table_width)
```
